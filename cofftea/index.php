<!DOCTYPE html>
<html lang="en">
<head>
	<?php require "../template/reset.php"; ?>
	<title>Cofftea Zone De Cavite</title>
	<?php require "../template/scriptsandstylesheet.php"; ?>
	<!-- <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta/css/bootstrap.min.css"> -->
	<link rel="stylesheet" type="text/css" href="css/coffteaSpecific.css">
</head>
<body>
	<div id="container">
		<header class="header">
			<section id="logocontainer">
				<img src="#" alt="cofftea zone">
			</section>
			<nav id="navbar">
				<ul>
					<li><a href="index.php">Home</a></li>
					<li><a href="aboutus.php">About Us</a></li>
					<li><a href="products.php">Menus</a></li>
					<li><a href="products.php">Gallery</a></li>
					<li><a href="events.php">Events</a></li>
					<li><a href="contactus.php">Contact Us</a></li>
				</ul>	
			</nav> <!-- end of navbar-->
		</header> <!-- end of header-->
		<section id="bannerimg">
			<div id="intro">
				<h2>No need to go far away to the metro!</h2>
				<h3>The coziest place in town to have your favorite coffee is finally here!</h3>
			</div>
		</section> <!-- end of bannerimg-->
		<section class="welcomeintro">
			<div class="imgWelcomeandtxtwelcomecont">
				<div id="imgWelcome">
					<p>See Our Menus.</p>
					<a href="#" alt="cofftea welcome img">Click Here</a>
				</div>
				<div id="txtwelcome">
					<h1>Welcome To Cofftea Zone</h1>
					<div class="txtintro">
						<p>Select from a wide range of coffee, tea or your favorite frappuccino! Gorge yourself with delicious food! Treat yourself with pastry sweets! Only here at CoffTea Zone!! We are WiFi Ready!</p>
					</div>
				</div>
			</div>
		</section>
		<footer id="footercontainer">
			<div class="footercontent">
				<div class="aboutinfocontainer">
					<p id="companyname"><span >Cofftea Zone</span></p>
					<p id="location"><span>851 Manila Cavite Road, Dalahican Cavite City,<br>Philippines.</span></p>
					<div id="phonenumber">
						<p>Contact Numbers:</p>
						<span>Globe: 09354075756 /</span>
						<span>Smart: 09954025039</span>
					</div>
					<div id="email">
						<p>email: <span>davidwithmore422@gmail.com</span></p>
					</div>
				</div>
				<div class="footernavbar">
					<div class="findus">
						<a href="#"><img src="" alt="facebook"></a>
					</div>
				</div>
			</div>
			<div id="copyrightcontainer">
				<p>&copy; Cofftea Zone &amp; Lago De Dalahican 2017</p>
			</div>
		</footer>
	</div>
	<script src="../js/jquery-3.2.1.js"></script>
	<!-- <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta/js/bootstrap.min.js></script> -->
</body>
</html>